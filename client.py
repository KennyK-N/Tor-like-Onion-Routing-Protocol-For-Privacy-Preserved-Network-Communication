...
class Client:
    def __init__(self):
        ...
        self.exchange_cond = (
            threading.Condition()
        )  # condition to synchronize key exchange process
                ...
                with open(file_path, "r") as f:
                    ...
                    if len(parts) >= 4:
                        ...
                        proxies[proxy_id] = {
                            "host": host,
                            "port": port,
                            "verification_key": crypto_utils.load_public_key(
                                verification_key
                            ),
                        }
    ...
    def listen_to_proxy(self, circuit, proxies):
        ...
        try:
            ...
            while self.running:
                if (
                    self.num_non_exchange_message_recv
                    == MAX_NUM_NON_EXCHANGE_MESSAGE_RECV_PER_SESSION
                ):
                    break
                ...
                if key_exchange_num < len(
                    circuit
                ):  # If this is a key exchange packet, perform key exchange and store the symmetric key
                    relay_num = 0
                    for i in range(key_exchange_num):
                        iv = packet.iv
                        packet = packet.payload
                        cipher = crypto_utils.create_cipher(
                            self.dh_key_info[i]["symm_key"], iv
                        )
                    ...
                    if not crypto_utils.verify_signature(
                        verification_key,
                        packet.payload["public_key"],
                        packet.payload["key_signature"],
                    ):
                        raise Exception(
                            "Invalid signature on public key from relay {relay_num+1}"
                        )
                    if not crypto_utils.verify_signature(
                        verification_key,
                        packet.payload["salt"],
                        packet.payload["salt_signature"],
                    ):
                        raise Exception(
                            "Invalid signature on salt from relay {relay_num+1}"
                        )
                    ...
                    for i in range(len(circuit)):
                        if outer_packet.exchange == False:
                            h = hmac.HMAC(
                                self.dh_key_info[i]["symm_key"], hashes.SHA256()
                            )
                        ...
                        cipher = crypto_utils.create_cipher(
                            self.dh_key_info[i]["symm_key"], packet.iv
                        )
                        decrypted_payload = crypto_utils.aes_decrypt(
                            cipher, packet.payload
                        )

                        if (
                            i == len(circuit) - 1
                        ):  # If this is the last layer, print the response
                            packet = PacketFormat.to_obj_rep(decrypted_payload)
    ...
    def key_exchange(self, circuit, proxies):
            ...
            try:
                if len(self.dh_key_info) == 0:
                    for i in range(len(circuit)):
                        ...
                        self.dh_key_info.append(
                            {
                                "private_key": private_key,
                                "public_key": public_key,
                                "symm_key": None,
                            }
                        )  # symm_key will be filled after key exchange response is received
                        ...
                        outer_packet = PacketFormat.Onion_Packet(
                        ...
                        self.proxy_sock.sendall(packet)
                        self.bytes_send += len(packet)
                        # Wait for response before sending next key exchange packet
                        self.exchange_cond.wait_for(
                            lambda: self.dh_key_info[i]["symm_key"] is not None
                        )
        ...
        self.key_exchange_time = (
            key_exchange_time_end - key_exchange_time_begin
        ).total_seconds() * 1000
    ...
    def create_packet(
        ...
        for i in range(dst_num - 1, -1, -1):
            if i == len(
                circuit
            ):  # If sending to server, innermost packet has server address and port
            ...
            if (
                i != 0
            ):  # If this is not the outermost packet, encrypt with the corresponding symmetric key for relay i
                cipher = crypto_utils.create_cipher(
                    self.dh_key_info[i - 1]["symm_key"], cur_iv
                )
                cur_payload = crypto_utils.aes_encrypt(
                    cipher, PacketFormat.to_bytes_rep(packet)
                )
            else:
